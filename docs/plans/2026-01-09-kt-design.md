...
`kt` stores tickets as markdown files with YAML frontmatter in `.tickets/`. Designed for AI agents to easily search and manipulate without dumping large JSON blobs into context windows.
...
│   │   └── ticket_test.go
│   ├── store/
│   │   ├── store.go             # Filesystem ops (.tickets/)
...
```go
type Store struct {
    Dir string // defaults to ".tickets"
}
